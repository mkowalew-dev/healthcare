...
| `Smart Care — VNS Health` | API | Network / ALB / TLS | 5 min |
| `Smart Care — SCFP Health` | API | Network / ALB / TLS | 5 min |
| `Smart Care — CPM Health` | API | Network / ALB / TLS | 5 min |
...
**Test name:** `Smart Care — VNS Health`
...
> **Protocol:** VNS is user-facing and served over HTTPS. Verify the response body's `status` field value against the live environment before saving the assertion — the SCFP health endpoint returns `"status":"ok"` (not `"healthy"`); confirm VNS follows the same convention.
...
**Test name:** `Smart Care — SCFP Health`
**Test type:** HTTP Server
**Timeout:** 5 s
**Frequency:** 1 min
**URL:** `http://scfp.pseudo-co.com/health`
**Method:** GET

> **Protocol:** SCFP `/health` is served over HTTP — confirmed from live response. The endpoint returns a full facility stats payload (`total_rooms`, `active_alerts`, `virtual_sitters_active`, etc.), not just a status flag, so the assertions below check operational fields rather than a generic status string.
...
| Response Body | contains | `"virtual_sitters_active"` |

*Alert rule:* HTTP ≠ 200, or response time > 500 ms. Set **"Error Type = Any"**.

---

### Smart Care — CPM Liveness (HTTP Server)

**Test name:** `Smart Care — CPM Health`
**Test type:** HTTP Server
**Timeout:** 5 s
**Frequency:** 1 min
**URL:** `http://cpm.pseudo-co.com/health`
**Method:** GET

> **Protocol:** Confirm HTTP vs HTTPS against the live CPM environment — use the same protocol as confirmed for SCFP. Adjust assertions once the live response shape is verified.
...
> **403 from Azure Application Gateway — HTTP version mismatch**
>
> The infrastructure uses a **Standard_v2 Azure Application Gateway** (no WAF). Azure Application Gateway uses the `Host` header to match incoming requests to backend routing rules. **HTTP/1.0 does not require a `Host` header**, so App Gateway cannot match the request to a routing rule and returns 403 by default.
>
> ThousandEyes **HTTP Server** tests default to HTTP/1.0. ThousandEyes **API Tests** use HTTP/1.1, which always includes a `Host` header — which is why the API Test to the same `http://scfp.pseudo-co.com/health` endpoint returns 200 while the HTTP Server test returns 403.
>
> **Fix — set HTTP Version to 1.1 in the HTTP Server test:**
> HTTP Server test → **Advanced Settings → HTTP Version → 1.1**
>
> Apply the same setting to the CPM Liveness HTTP Server test. Once both tests send HTTP/1.1, the App Gateway receives a proper `Host: scfp.pseudo-co.com` header, matches the routing rule, and forwards the request to the SCFP backend.

> **Path Visualization benefit:** Assigning all three HTTP Server tests (VNS, SCFP, CPM) to the same Enterprise Agent makes the Path Visualization views directly comparable. If VNS is unreachable but SCFP and CPM are healthy, the divergence point in the path immediately localises the fault to the VNS-specific route in the App Gateway, not a shared network segment.
...
>
> **Note:** Step 1 calls `https://careconnect.pseudo-co.com/api/auth/login` — a different host from the SCFP base URL. ThousandEyes API tests support per-step URLs. SCFP validates the returned JWT against the same shared `JWT_SECRET` as the CareConnect backend; no separate SCFP-specific login endpoint is needed.

---

**Step 1 — Login (provider)**

Authenticates as the demo provider against the CareConnect backend and extracts the JWT used as the Bearer token for every subsequent SCFP step.
...
**Step 7 — Alert acknowledge (write path)**

Acknowledges the alert extracted in step 6. SCFP continuously generates new alerts, so the queue always has unacknowledged items and `scfpAlertId` is reliably populated. A `404` is acceptable — it occurs when the alert was acknowledged between step 6 and step 7 on a concurrent run; configure the assertion to treat `200` **or** `404` as a pass.
...
**Step 8 — AI staff workflow recommendations**
...
>
> **Note:** Step 1 calls `https://careconnect.pseudo-co.com/api/auth/login`. CPM validates the returned JWT against the same shared `JWT_SECRET` as the CareConnect backend; no separate CPM-specific login endpoint is needed.

---

**Step 1 — Login (provider)**

Authenticates as the demo provider against the CareConnect backend and extracts the JWT used as the Bearer token for every subsequent CPM step.
...
**Step 5 — NEWS2 early warning score**
...
| SCFP Room Intelligence | Any step HTTP ≠ 200 (step 7 also accepts 404), or `video_feed_status` / `recommendation` missing | Total > 10 s |
...
2. **CPM — Device Health**: a separate alert rule is not configurable on an API test body assertion alone — instead, monitor `"signal":"fair"` or low `battery` in step 7 by setting the body assertion to `not contains "offline"` on `status`. Configure the alert to trigger when this assertion fails on two or more consecutive test runs — a transient offline status (device rebooting) should not page; a persistently offline device should.
...
# Obtain CareConnect provider JWT (used for SCFP and CPM API calls)
TOKEN=$(curl -s -X POST $BASE/api/auth/login \
  -H 'Content-Type: application/json' \
  -d '{"email":"provider@careconnect.demo","password":"Demo123!"}' \
  | python3 -c "import sys,json; print(json.load(sys.stdin)['token'])")
...
if [ -z "$TOKEN" ]; then
  echo "  ✗  CareConnect login failed — no token returned (needed for SCFP/CPM auth)"
  exit 1
else
  printf "  ✓  %-55s token obtained\n" "CareConnect provider login"
fi
...
ROOM=room-301
check "Rooms list"                "$SCFP/api/rooms"
check "Room by ID"                "$SCFP/api/rooms/$ROOM"
check "Fall detection events"     "$SCFP/api/events/falls?limit=20"
check "Virtual sitters list"      "$SCFP/api/sitters"
SCFP_ALERT_ID=$(curl -s -H "Authorization: Bearer $TOKEN" \
  "$SCFP/api/alerts?acknowledged=false&limit=20" \
  | python3 -c "import sys,json; d=json.load(sys.stdin); print(d['alerts'][0]['id']) if d.get('alerts') else print('')" 2>/dev/null)
check "Alert queue"               "$SCFP/api/alerts?acknowledged=false&limit=20"
if [ -n "$SCFP_ALERT_ID" ]; then
  ack_code=$(curl -s -o /dev/null -w "%{http_code}" -X PATCH \
    -H "Authorization: Bearer $TOKEN" "$SCFP/api/alerts/$SCFP_ALERT_ID/ack")
  [ "$ack_code" = "200" ] || [ "$ack_code" = "404" ] \
    && printf "  ✓  %-55s %s (200 or 404 ok)\n" "Alert acknowledge" "$ack_code" \
    || printf "  ✗  %-55s %s\n" "Alert acknowledge" "$ack_code"
fi
check "AI staff workflow"         "$SCFP/api/staff/workflow"

echo ""
echo "=== CPM Patient Monitoring ==="
check "Patients list"             "$CPM/api/patients"
check "Patient by ID"             "$CPM/api/patients/$CPM_PT"
check "Patient vitals"            "$CPM/api/patients/$CPM_PT/vitals?limit=12"
check "NEWS2 EWS"                 "$CPM/api/patients/$CPM_PT/ews"
check "ADL score"                 "$CPM/api/patients/$CPM_PT/adl"
check "Monitoring devices"        "$CPM/api/devices"
check "CPM alert queue"           "$CPM/api/alerts?limit=50"
