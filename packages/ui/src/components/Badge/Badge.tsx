...
export function LabStatusBadge({ status }: { status: string }) {
  ...
  return <Badge variant={variantMap[status] ?? 'gray'}>{labels[status] ?? status}</Badge>;
}
...
export function MedStatusBadge({ status }: { status: string }) {
  ...
  return <Badge variant={variantMap[status] ?? 'gray'}>{status.replace(/_/g, ' ')}</Badge>;
}
...
export function AllergySeverityBadge({ severity }: { severity: string }) {
  ...
  return (
    <Badge variant={variantMap[severity] ?? 'gray'}>
